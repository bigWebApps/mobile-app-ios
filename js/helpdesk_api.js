...
function api_login(email, pass, success_function) {
    $.getJSON(getapicommandurl('login'), { LoginName: email, Password: pass }, success_function).error(function () { alert('Server Error') });
}
function api_organizations(success_function) {
    $.getJSON(getapicommandurl('org'), null, success_function).error(function () { alert('Server Error') });
}

function api_set_organization(key, success_function) {
    $.getJSON(getapicommandurl('setorg', key), null, success_function).error(function () { alert('Server Error') });
}

function api_instances(success_function) {
    $.getJSON(getapicommandurl('inst'), null, success_function).error(function () { alert('Server Error') });
}

function api_set_instance(key, success_function) {
    $.getJSON(getapicommandurl('setinst', key), null, success_function).error(function () { alert('Server Error') });
}